...
{
    public class PostsInMemoryRepository : IPostRepository
    {
        private readonly CategoriesInMemoryRepository categoriesInMemoryRepository;
        public PostsInMemoryRepository(CategoriesInMemoryRepository categoriesInMemoryRepository)
        {
        ...
        public IEnumerable<Post> GetPosts(bool loadCategory = false)
        {
            ...
            else
            {
                if (_posts != null && _posts.Count > 0)
                {
                    _posts.ForEach(post =>
                    {
                        post.Category = categoriesInMemoryRepository.GetCategoryById(post.CategoryId.Value);
                    });
